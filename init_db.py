...
def upsert_demo_employee_user(email: str, password: str) -> None:
    # First create the employee record (employee_id will be auto-generated)
    employee = Employee.query.filter_by(email=email).first()
    if employee is None:
        employee = Employee(
            # employee_id will be auto-generated by the database
            first_name="Demo",
            last_name="Employee",
            email=email,
        ...
        db.session.flush()  # This gets the auto-generated employee_id
    
    # Now create the user record with the employee_id
    ...
    if user is None:
        ...
        user.employee_id = employee.employee_id  # Update with the correct ID
...
def seed_departments() -> None:
    ...
    departments = [
        ...
        },
        {
            ...
            "description": "Manages market research, advertising, brand management, and customer engagement."
        },
        {
...
def seed_wage_masters() -> None:
    """Seed default wage master data"""
    wage_masters = [
        {
        ...
        {
            "salary_code": "MUMMANAGERMAR",
            "site_name": "Mumbai Office",
            "rank": "Manager",
            "state": "Maharashtra",
            "base_wage": 75000,
            "skill_level": "Managerial"
        },
        {
            "salary_code": "BANANALYSTKAR",
    ...
    for wm_data in wage_masters:
        wm = WageMaster.query.filter_by(salary_code=wm_data["salary_code"]).first()
        if wm is None:
            wm = WageMaster(
                salary_code=wm_data["salary_code"],
                site_name=wm_data["site_name"],
                rank=wm_data["rank"],
                state=wm_data["state"],
                base_wage=wm_data["base_wage"],
                skill_level=wm_data["skill_level"],
                is_active=True,
                created_by="_init_db"
            )
            db.session.add(wm)
    
    print("✅ Seeded wage masters")
    ...
    with app.app_context():
        try:
            ...
            try:
                ...
                print(f"⚠️  Sequence setup warning: {e}")
                db.session.rollback()
            ...
            if seed_demo:
                ...
                # Seed users (this will create the demo employee with auto-generated ID)
