...
def validate_attendance_data(df, date_columns, month, year):
    ...
    valid_statuses = ['P', 'A', 'O', 'R', 'Present', 'Absent', 'OFF', 'Reliever', 'Reliever Duty', '']
                ...
                if pd.notna(value):
                    ...
                    if status_str not in [s.upper() for s in valid_statuses]:
                        results['invalid_statuses'].append({
                            ...
                            'column': col,
                            'value': value,
                            'error': f"Invalid attendance status '{value}'. Must be P, A, O, R, Present, Absent, OFF, or Reliever"
                        })
